...
//		itemTable.toStream().foreach((String itemName, Item item) -> System.out.printf("Item info %s-%s-%s-%s\n", item.getItemName(), item.getAddress(), item.getType(), item.getPrice()));
        KTable<String, OrderUserItem> kTable = orderStream
                ...
                .through(Serdes.String(), SerdesFactory.serdFrom(OrderUserItem.class), (String key, OrderUserItem orderUserItem, int numPartitions) -> (orderUserItem.getItemName().hashCode() & 0x7FFFFFFF) % numPartitions, "orderitem-repartition-by-item")
                .map((String item, OrderUserItem orderUserItem) -> KeyValue.<String, OrderUserItem>pair(String.format("%s, %s",orderUserItem.getUserAddress(), orderUserItem.getGender()), orderUserItem))
                .groupByKey(Serdes.String(),SerdesFactory.serdFrom(OrderUserItem.class))
                .reduce((OrderUserItem v1, OrderUserItem v2) -> OrderUserItem.add2OrderUser(v1,v2) , "grp-state-store");

        kTable.foreach((str, dou) -> System.out.printf("%s-%s\n", str, dou));
        kTable
                .toStream()
                .map((String key, OrderUserItem total) -> new KeyValue<String, String>(key, String.format("%s, %s, %s", key, total.getQuantity(), total.getItemAmount())))
                .to("grp-addr-gender-qty-total");
	...
	public static class OrderUserItem {
        ...
        public static OrderUserItem add2OrderUser(OrderUserItem item1, OrderUserItem item2) {
            OrderUserItem  item= new OrderUserItem();
            item.userName = item1.userName;
            item.itemName = item1.itemName;
            item.userAddress = item1.userAddress;
            item.gender = item1.gender;
            item.quantity = item1.getQuantity() + item2.getQuantity();
            item.itemAmount = item1.getItemAmount() + item2.getItemAmount();
            item.itemPrice = item.itemAmount/item.quantity;
            return item;
        }
